#!/bin/bash
# Dynu + Traefik 自动化设置脚本
# 用于在AWS EC2上快速部署n8n with HTTPS

set -e

echo "=========================================="
echo "  Dynu + Traefik 自动化设置"
echo "=========================================="
echo ""
...
if [ ! -f ".env" ]; then
    ...
    echo ""
    echo "1. Dynu 用户名:"
    echo "   - 访问 https://www.dynu.com"
    echo "   - 登录后，在控制台可以看到你的用户名"
    echo "   - 在.env文件中设置: DYNU_USERNAME=your-username"
    echo ""
    echo "2. Dynu 密码:"
    echo "   - 你的Dynu账户密码"
    echo "   - 在.env文件中设置: DYNU_PASSWORD=your-password"
    echo ""
    echo "3. Dynu域名:"
    echo "   - 在Dynu控制台添加或使用已有域名"
    echo "   - 完整域名格式: yourname.dynu.com 或 your-custom-domain.com"
    echo "   - 在.env文件中设置: DYNU_DOMAIN=yourname.dynu.com"
    echo "   - 同时设置: N8N_DOMAIN=yourname.dynu.com"
    echo ""
    echo "4. Let's Encrypt邮箱:"
    echo "   - 用于SSL证书通知"
...
# 步骤6: 设置Dynu自动更新
echo "[6/6] 设置Dynu自动更新..."
if [ -f "update-dynu.sh" ]; then
    chmod +x update-dynu.sh
    
    # 检查crontab是否已存在
    if ! crontab -l 2>/dev/null | grep -q "update-dynu.sh"; then
        SCRIPT_PATH=$(pwd)/update-dynu.sh
        (crontab -l 2>/dev/null; echo "*/5 * * * * $SCRIPT_PATH >> /tmp/dynu-cron.log 2>&1") | crontab -
        echo "✓ 已添加Dynu自动更新到crontab（每5分钟更新一次）"
    else
        echo "✓ Dynu自动更新已配置"
    fi
    ...
    # 立即执行一次更新
    echo "执行首次Dynu更新..."
    ./update-dynu.sh || echo "警告: Dynu更新失败，请检查配置"
else
    echo "警告: update-dynu.sh文件不存在"
fi
echo ""
...
echo "   - DYNU_PASSWORD"
echo "   - DYNU_DOMAIN"
echo "   - N8N_DOMAIN (与DYNU_DOMAIN相同)"
